"""Run EagleEye's real Chrome extension against the bundled authorized target.
...
sample target and writes publication-safe screenshots and a JSON receipt.
"""
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        target.goto(TARGET_URL, wait_until="domcontentloaded", timeout=60_000)
        expect(target).to_have_title(re.compile("EagleEye Local QA Lab"), timeout=20_000)

        # OFF proof: a real navigation must not create a browser-agent session.
        target.get_by_role("link", name="Sample Page", exact=True).first.click()
        ...
        popup.locator("#session-name").fill("Authorized Local Journey")
        ...
        while time.monotonic() < grant_deadline:
            ...
            if active_tabs and active_tabs[0].get("url", "").startswith(TARGET_URL):
                break
            time.sleep(0.25)
        else:
            raise RuntimeError(
        ...
        target.get_by_role("link", name="Sample Page", exact=True).first.click()
        target.wait_for_load_state("domcontentloaded", timeout=60_000)
        target.wait_for_timeout(2_500)
        target.screenshot(path=SCREENSHOTS / "03-target-observed.png", full_page=True)
    ...
    evidence_path = EVIDENCE / "extension-browser-e2e.json"

...
実WordPressでのfresh Chrome検証（2026-07-17）は、拡張OFF時の記録0件、ON後4観測、5ケース生成、AI fallbackなし、ケース品質100点、Replay PASSを記録しています。公開可能な機械証跡は[extension-wordpress-e2e.json](docs/build-week/evidence/extension-wordpress-e2e.json)です。
...
起動後、`http://127.0.0.1:8766/` を開き、**デモを開始**を押します。ローカルのWordPressが`127.0.0.1:8888`で動作していれば実WordPressを優先し、なければ同梱のログイン不要デモへ切り替わります。

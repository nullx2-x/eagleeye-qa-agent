...
- scene: 実拡張の記録中画面と実Authorized target操作を並べ、明示ONから通常操作まで見せる
- voiceover: "Chrome拡張をON。ユーザーはAuthorized targetをいつもどおり操作する。OFFの間は何も記録しない。"
- duration: 18s
...
- asset_candidates: assets/screenshots/extension-recording.png — 明示的な記録中状態; assets/screenshots/dashboard.png — 実Authorized target Sample Page
- focal: assets/screenshots/dashboard.png
- roles: extension-recording = supporting（制御面） · target-observed = cutout（操作対象）
- sfx: none

Adapt: 一つの製品surfaceが操作に応じて状態を進める署名構造を、左の拡張状態と右のAuthorized target実画面の同期で置き換える。surfaceは全編で保持し、画面だけを段階的に読ませる。
Scene 1 (0.0–4.2s): extension-recordingを左42%、target-observedを右58%のsplitへedge slide-in + settle。最初は拡張の「記録中」だけをcoral hairlineで囲み、「明示的にON」をmonoラベルで表示する。3 depth layers、bottom 17%は空ける。
Scene 2 (4.2–9.8s): Authorized target側へ短いcoordinate zoom（`coordinate-target-zoom`）を行い、Sample Pageの見出しと本文を順にhairline markerで示す。実画面はscale-swapせずpersistent、視線だけを移す。
...
- asset_candidates: assets/screenshots/dashboard.png — 観測対象画面; assets/screenshots/dashboard.png — 同一セッションの運用画面
- focal: assets/screenshots/dashboard.png
- roles: target-observed = background（dim 38%） · dashboard = supporting（同一セッション）
- sfx: none

Adapt: N itemsが短い距離から各slotへassembleする署名動作を、安全な三文脈の3-card gridへ適用する。共通中心から爆発させず、各カードが自分の位置へ入る。
Scene 1 (0.0–3.6s): target-observedをfull-bleed dim 38%で置き、「EagleEyeが見るもの」だけを上三分割へ表示。中央のgrid領域は空のまま。
...
- scene: ReplayボタンからAuthorized target導線が再生され、モデルではなくPlaywrightが判定する
...
- asset_candidates: assets/screenshots/replay-result.png — Replay操作と結果; assets/screenshots/dashboard.png — 再現された最終画面
- focal: assets/screenshots/replay-result.png
- roles: replay-result = cutout（操作面） · target-observed = supporting（再現結果）
- sfx: none
...
Scene 3 (10.0–15.6s): target-observedを右側へcard morph-anchor（`card-morph-anchor`）で展開し、Sample Page見出しをhairline markerで示す。左にrunner、右に最終画面のasymmetric 45/55。
...
- voiceover: "今回の実Authorized target Replayは1,745ミリ秒でPASS。画像と動画にbyte数、時刻、SHA-256を残す。"
- duration: 18s
- transition_in: crossfade
- status: animated
...
- blueprint: dataviz-countup (Adapt)
- asset_candidates: assets/screenshots/report.png — 実行証跡を含むHTMLレポート; assets/screenshots/dashboard.png — Replay最終画面
- focal: assets/screenshots/report.png
- roles: report = cutout（実測レポート） · target-observed = supporting（最終画面証跡）
- sfx: none
...
Scene 3 (10.6–14.4s): target-observedを左supportingへ置き、report内のtimestampとSHA-256を右側へmono railで一行ずつreveal。hashは先頭12文字＋末尾8文字だけを表示し、全文はレポートに残す。

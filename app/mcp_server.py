...
def discover_project_qa(project_root: str, authorized: bool = False) -> dict:
    """Discover tests, lint, type checks, and builds in an explicitly authorized local project."""
    if not authorized:
        raise PermissionError("Explicit project test authorization is required")
...
def run_project_qa(
    ...
    """Run discovered QA suites without a shell and return hashed local evidence plus a gate."""
    if not authorized:
        raise PermissionError("Explicit project test authorization is required")

...
| Codex provider | Structured model turns through Codex App Server | App Server owns authentication; EagleEye does not receive the token | **IMPLEMENTED / LIVE-VERIFIED** - the Authorized target browser session returned five schema-valid cases from `codex-agent` / `gpt-5.6-terra`, available `true`, fallback `false` |
...
The implemented design uses the following controls. Static extension checks, the Python suite, and a fresh unpacked-Chromium Authorized target run pass; public release still requires the separate publication gate.
...
2. Four ordered observations from the previously tested local Authorized target site.

...
from app.project_qa_api import router as project_qa_router
...
def test_secret_spanning_output_read_boundary_is_redacted(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    executable = Path(sys.executable).name
    marker_value = "split-boundary-secret"
    project = _manifest_project(
        tmp_path,
        [executable, "-c", f"print('X' * 65530 + ' api_key={marker_value}')"],
    )
    run_root = tmp_path / "runs"
    monkeypatch.setenv("EAGLEEYE_PROJECT_ROOTS", str(tmp_path))
    monkeypatch.setattr("app.project_qa.RUNS", run_root)
    ...
    evidence = Path(report.results[0].evidencePath).read_text(encoding="utf-8")
    assert marker_value not in evidence
...
def test_project_qa_routes_are_registered_once() -> None:
    included_routers = [getattr(route, "original_router", None) for route in app.routes]
    assert included_routers.count(project_qa_router) == 1
